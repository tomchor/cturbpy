...
problem.substitutions["t_xx"] = ("2/3*(2*dx(u) - wz)")
problem.substitutions["t_xz"] =("dx(w) + uz")
problem.substitutions["t_zz"] =("2/3*(2*wz - dx(u))")
...
problem.add_equation("dt(u) - ν*dx(t_xx) - ν*dz(t_xz)       = - w*dz(u) - u*dx(u)   + ν*t_xx*dx(ρ)/ρ + ν*t_xz*dz(ρ)/ρ")
problem.add_equation("dt(w) - ν*dz(t_zz) - ν*dx(t_xz)       = - w*wz    - u*dx(w)   + ν*t_zz*dz(ρ)/ρ + ν*t_xz*dx(ρ)/ρ")
problem.add_equation("dt(T) - 1/Cv*(κ*(dz(Tz) + dx(dx(T)))) = - w*Tz    - u*dx(T) - (γ-1)*T*(wz + dx(u)) + 1/Cv*(κ*(Tz*dz(ρ)/ρ + dx(T)*dx(ρ)/ρ) \
                                                                                    + ν*t_zz*wz + ν*t_xx*dx(u) + ν*t_xz*uz + ν*t_xz*dx(w))")
...
snapshots.add_task('0.8*T*ρ', name='P')
